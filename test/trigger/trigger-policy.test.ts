...
describe("trigger-policy: decideTrigger", () => {
  ...
  test("PostToolUse: near-duplicate takes precedence over cadence when both are true", () => {
    const decision = decideTrigger({
      hookEvent: "PostToolUse",
      postToolUseSuccessCount: 8, // a cadence multiple of 4
      cadenceN: 4,
      isNearDuplicate: true,
    });
  ...
  test("PostToolUse: fires on cadence multiples", () => {
    for (const step of [4, 8, 12, 40]) {
      const decision = decideTrigger({ hookEvent: "PostToolUse", postToolUseSuccessCount: step, cadenceN: 4, isNearDuplicate: false });
  ...
  test("cadenceN=1 triggers on every PostToolUse call", () => {
    for (const step of [1, 2, 3]) {
      const decision = decideTrigger({ hookEvent: "PostToolUse", postToolUseSuccessCount: step, cadenceN: 1, isNearDuplicate: false });
  ...
  test("phase2Eligible is true for every non-PreCompact trigger, triggered or not", () => {
    const notDue = decideTrigger({ hookEvent: "PostToolUse", postToolUseSuccessCount: 1, cadenceN: 4, isNearDuplicate: false });
